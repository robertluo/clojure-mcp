  ...
  {:type :object
   :properties {:file_path {:type :string
                ...
                :old_string {:type :string
                             :description "The text to replace (must match the file contents exactly, including all whitespace and indentation). Empty string for new file creation."}
...
(defmethod tool-system/validate-inputs :file-edit [{:keys [nrepl-client-atom]} inputs]
  (let [{:keys [file_path old_string new_string]} inputs
    ...
    (let [validated-path (utils/validate-path-with-client file_path nrepl-client)]
      ...
      ;; For new file creation, check if file already exists
      (when (and (empty? old_string) (.exists (io/file validated-path)))
        (throw (ex-info "Cannot create new file - file already exists."
                        {:inputs inputs})))
